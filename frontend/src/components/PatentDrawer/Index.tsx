...
function AssigneeChip({
  ...
  if (list.length === 0)
    return (
      <span className="text-xs text-slate-500 italic">Unknown Assignee</span>
    );
  ...
  return (
    <div className="relative group flex items-center">
      ...
      <div className="absolute top-full left-0 mt-2 hidden group-hover:flex flex-col bg-slate-800 border border-slate-700/80 p-3 rounded-lg shadow-2xl z-[100] min-w-[220px] max-w-[350px] backdrop-blur-xl">
        <div className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest mb-2 border-b border-slate-700/50 pb-1.5">
          All Assignees
        </div>
        <div className="text-xs text-slate-300 max-h-[200px] overflow-y-auto custom-scrollbar flex flex-col gap-1.5">
          {list.map((a, i) => (
            <span key={i} className="whitespace-normal leading-snug">
              {a}
            </span>
          ))}
        </div>
  ...
  return (
    <div className="flex flex-col">
      ...
      >
        ...
        <span
          className={`text-sm ${level === 0 ? "text-slate-600 font-semibold" : "text-slate-600"}`}
        >
          {name}
        </span>
      </div>
...
function TaxonomyTree({ taxonomies }: { taxonomies: any[] }) {
  if (!taxonomies || taxonomies.length === 0) {
    return (
      <p className="text-sm text-slate-700 italic px-2">
...
const PatentDrawer = ({ patent, onClose }: PatentDetailDrawerProps) => {
  ...
  const filteredBwdCit = (patent?.backward_citations || []).filter(
    (c: any) =>
      c.publication_number?.toLowerCase().includes(citSearch.toLowerCase()) ||
      c.assignee?.toLowerCase().includes(citSearch.toLowerCase()),
  );
            ...
            <div className="flex items-center gap-6 px-6 py-3 bg-slate-100 border-b border-slate-700/40 shrink-0">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-emerald-600">
                  {patent.forward_citations?.length || 0}
                </span>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-rose-600">
                  {patent.backward_citations?.length || 0}
                </span>
              </div>
            </div>
                    ...
                    <div className="max-h-[300px] overflow-y-auto custom-scrollbar pr-1 space-y-3">
                      {(() => {
                        ...
                        return sortedList.map((assignee: any, i: number) => {
                          const topicEvals =
                            assignee.topic_evals ||
                            (assignee.topic_eval ? [assignee.topic_eval] : []);
                          const subtopicEvals =
                            assignee.subtopic_evals ||
                            (assignee.subtopic_eval
                              ? [assignee.subtopic_eval]
                              : []);
                          ...
                          const maxTopicScore =
                            topicEvals.length > 0
                              ? Math.max(
                                  ...topicEvals.map((e: any) => e.score || 0),
                                )
                              : 0;
                          const maxSubtopicScore =
                            subtopicEvals.length > 0
                              ? Math.max(
                                  ...subtopicEvals.map(
                                    (e: any) => e.score || 0,
                                  ),
                          ...
                          return (
                            <div
                              key={`ranked-${i}`}
                              className="bg-slate-100 border border-slate-700/50 rounded-lg p-3"
                            >
                              ...
                              <div className="space-y-3">
                                {topicEvals.map((evalObj: any, idx: number) =>
                                  evalObj.reason ? (
                                    <div key={`teval-${idx}`}>
                                    ...
                                    evalObj.reason ? (
                                      <div key={`seval-${idx}`}>
                                        <div className="text-[10px] font-semibold text-slate-500 uppercase flex items-center flex-wrap gap-x-1">
                                          ...
                                          <span className="font-bold text-amber-500/80 normal-case">
                                            ({evalObj.score}/10)
                                          </span>
                                        </div>
                                        <p className="text-xs text-slate-500 italic mt-0.5">
              ...
              <div className="relative rounded-xl border border-slate-700/50 overflow-hidden">
                ...
                <div className="pl-5 pr-4 pt-4 pb-4">
                  ...
                  {isProcessing ? (
                    ...
                    (!patent.kyp_classifications ||
                      patent.kyp_classifications.length === 0) ? (
                    <EmptyState title="No KYP Data Available" />
                  ) : (
                    <div className="space-y-4">
                      {patent.kyp_score_data && (
                        <div className="bg-slate-800 rounded-lg border border-slate-700/50 p-3">
                          <div className="text-[10px] font-bold text-slate-200 uppercase mb-2">
                            Scoring Parameters
                          </div>
                          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                            {Object.entries(patent.kyp_score_data).map(
                              ([key, value]) => {
                                if (
                                  typeof value === "object" ||
                                  key === "classifications" ||
                                ...
                                return (
                                  <div
                                    key={key}
                                    className="bg-slate-900/50 rounded p-2 flex flex-col justify-center"
                                  >
                                    <div
                                    ...
                                    <div
                                      className="text-xs font-semibold text-slate-300 mt-0.5 truncate"
                                      title={String(value)}
                                    >
                                      {String(value)}
                                    </div>
                        ...
                        patent.kyp_classifications.length > 0 && (
                          <div>
                            ...
                            <div className="max-h-[200px] overflow-y-auto custom-scrollbar space-y-1.5 pr-1">
                              {patent.kyp_classifications.map(
                                (c: any, idx: number) => (
                                  <div
                                    key={idx}
                                    className="bg-slate-800 border border-slate-700/30 rounded p-2 flex gap-2 items-start"
                                  >
              ...
              <div className="relative rounded-xl border border-slate-700/50 overflow-hidden">
                ...
                <div className="pl-5 pr-4 pt-4 pb-4">
                  <h3 className="text-[11px] font-bold text-sky-400 uppercase tracking-widest mb-4 flex items-center justify-between">
                    ...
                    <span className="flex gap-2">
                      <button
                        onClick={() => scrollToRef(fwdCitRef)}
                        className="px-2 py-0.5 rounded-full bg-emerald-600 text-emerald-100 text-[12px] font-bold border border-emerald-500/20 hover:bg-emerald-500/25 transition-colors cursor-pointer"
                      >
                        ↑ {patent.forward_citations?.length || 0} Fwd
                      </button>
                      <button
                        onClick={() => scrollToRef(bwdCitRef)}
                        className="px-2 py-0.5 rounded-full bg-rose-500/15 text-rose-600 text-[12px] font-bold border border-rose-500/20 hover:bg-rose-500/25 transition-colors cursor-pointer"
                      >
                  ...
                  ) : !patent.forward_citations?.length &&
                    !patent.backward_citations?.length ? (
                      ...
                      <div className="max-h-[350px] overflow-y-auto custom-scrollbar pr-1 space-y-1">
                        ...
                        {filteredFwdCit.length > 0 ? (
                          <>
                            ...
                            {filteredFwdCit.map((c: any, i: number) => (
                              <div
                                key={`fwd-${i}`}
                                className="flex items-start gap-2.5 py-2 border-b border-slate-700/30 last:border-0 group"
                              >
                                ...
                                <div className="flex flex-col min-w-0">
                                  <span className="text-xs font-semibold text-slate-600 hover:text-emerald-600 transition-colors">
                                    {typeof c === "string"
                                      ? c
                                      : c.patent_number || c.publication_number}
                                  </span>
                                  {typeof c === "object" && (
                                    <AssigneeChip
                          ...
                          <div className="flex flex-col items-center justify-center py-6 px-4 border border-dashed border-emerald-500/20 rounded-lg bg-emerald-500/5 mb-4">
                            <span className="text-xs font-medium text-emerald-400/60 uppercase tracking-wider mb-1">
                              Forward Citations
                            </span>
                        ...
                        {filteredBwdCit.length > 0 ? (
                          <>
                            <div
                              ref={bwdCitRef}
                              className="text-[12px] px-2 py-5 font-bold text-rose-500 uppercase tracking-wider py-1.5 sticky top-0 bg-slate-800 backdrop-blur-sm mt-2 z-10"
                            >
                              Backward Citations
                            </div>
                          ...
                          <div className="flex flex-col items-center justify-center py-6 px-4 border border-dashed border-rose-500/20 rounded-lg bg-rose-500/5 mb-4">
                            <span className="text-xs font-medium text-rose-400/60 uppercase tracking-wider mb-1">
                              Backward Citations
                            </span>
                            <span className="text-[10px] text-slate-500">
                              None found
